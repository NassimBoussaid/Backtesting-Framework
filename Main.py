...
moving_average_strategy = MovingAverage(10, 90)
result = ahah.run(moving_average_strategy, 5, 90)

historical_data['Moving_Average_10'] = historical_data['AAPL UW Equity'].rolling(window=10).mean()
historical_data['Moving_Average_90'] = historical_data['AAPL UW Equity'].rolling(window=90).mean()

# Convert the Series 'result' to a DataFrame
merged_data = pd.DataFrame(result, columns=['Result'])
...
merged_data['Result'].plot(ax=ax, label='Signal (AAPL UW Equity)', linewidth=1, secondary_y=True)
